...
class MultiSelect:
    ...
    def func(self):
        if item.get('ident')== QnList[qnNumber]:
            resprocess = item.findall('resprocessing/respcondition{[loopCounter]}/setvar')
            score = resprocess.attrib['varname']
            if score == "D2L_Correct":
                    for v in item.findall('resprocessing/respcondition{[loopCounter]}/conditionvar'):
                       correct=v.find('varequal').text
                       AnsToBePrinted=ansIdList[correct]
                       if AnsToBePrinted.startswith("<p>"):
                            formattedAnsToBePrinted=AnsToBePrinted[3:-4]
...
class ShortAns:
    ...
    def func(self):
        if item.get('ident') == QnList[qnNumber]:
            for o in item.findall('/resprocessing/respcondition/conditionvar'):
...
class FillTB:
    def __init__(self,element):
        self.element = element
    def func(self):
    # for o in root.findall(f'./objectbank/section/item[@ident="{QnList[qnNumber]}"]'):
            LongAnswer=o.find('/resprocessing/respcondition/conditionvar/varequal').text
            file_html.write("<br><strong>Answer : {}   </strong><br>".format(LongAnswer))
class LongAns:
  def __init__(self,element):
      self.element = element
            ...
            class QnMcqTf:
                ...
                def func(self):
                    ...
                    if AnsToBePrinted.startswith("<p>"):
                        ...
                        ansDict[qnNumber+1].append(AnsToBePrinted)
                    return ansDict
            for itemRoot in root.findall('./objectbank/section/item'):
                QnList.append(itemRoot.attrib['ident'])
                b = itemRoot.find('/itemmetadata/qtimetadata/qti_metadatafield[2]')
                questiontype = b.find('fieldentry').text
                # check type of item 
                # use if else to instatiate a quesiton object based on the type. 
                if questiontype == "MCQ":
                    item = QnMcqTf(itemRoot)
                    item.func()
                    ...
                    item.func()
                elif questiontype == "Short Answer" or questiontype == "Multi-Short Answer":
                    item = ShortAns(itemRoot)
                    item.func()
                elif questiontype == "Long Answer":
                    item = LongAns(itemRoot)       
                elif questiontype == "Fill in the Blanks":
                    item = FillTB(itemRoot)
                elif questiontype not in accepted_types:
                    print("error")
                    ...
                    if qntype == "Short Answer" or qntype == "Multi-Short Answer" or qntype == "Fill in the Blanks":
                        for o in root.findall(f'./objectbank/section/item[@ident="{QnList[qnNumber]}"]'):
                                ...
                                if flow_element is not None:
                                    texts = [child.text.replace('response', '_____') for child in flow_element.iter()]
                                    # Joining the modified strings into a single string
                                    content = ''.join(texts)
                                    # print the concatenated content
                                    print(content)
                                else:
                                    print("Could not find the presentation/flow element in the XML file.")
                                quest = FillTB()
                                quest.FTB()
